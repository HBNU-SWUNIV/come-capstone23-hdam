...
# 분석을 진행할 시간
analysis_time = ['18시간전', '19시간전', '20시간전', '21시간전']
...
def CheckStartPage():
    ...
    while True:
        ...
        if split_article_time[0].isdigit() and int(split_article_time[0]) >= 18:
            break
        else:
            start_page += 1
...
def CrawlingToMultiprocessing(param1, page):
    ...
    while True:
        ...
        for j in range(1, 6):
            ...
            if split_article_time[0].isdigit() and int(split_article_time[0]) >= 22:
                escape_flag = True
                break
