...
def CrawlingToMultiprocessing(param1):
    ...
    while True:
        ...
        for j in range(1, 6):
            ...
            if split_article_time[0].isdigit() and int(split_article_time[0]) >= 6:
                escape_flag = True
                break
...
def DayKeywordCrawler_0to5():
    global keyword_dict
    global newsCount

    ###### 멀티스레드 사용
    with ThreadPoolExecutor(max_workers=8) as executor:
        res = [executor.submit(CrawlingToMultiprocessing, i) for i in range(1, 5)]
        ######
